<div class="card frontend-record record-card position-relative" style="width: 200px;height: 35rem;">
  ...
  <a href="#"><img class="card-img-top img-fluid" src="{{$record->photo ? asset($record->photo) : asset('img/default-avatar.jpg') }}" alt="Image placeholder" style="max-height:190px;min-height:190px;object-fit: cover;"></a>
  <div class="card-body">
    @if($record->checkBookedBy(auth()->user()->corporation->id ?? 0)
      <button class="btn btn-block btn-danger" id="choose-record-{{$record->id}}">Batal</button>
    @else
      <button class="btn btn-block btn-success" id="choose-record-{{$record->id}}">Pilih</button>
    @endif
    <a href="#">
...
<script>
    $(document).ready(function(){
        $('#choose-record-{{$record->id}}').on('click', function(e) {
            $.ajax({
                type: "POST",
                url: '{{route("frontend.bookings.pickRecord")}}',
                data: {
                ...
                success: function (response) {
                  if(response.isPicked){
                    ...
                    console.log($(this));
                    $('#choose-record-{{$record->id}}').removeClass( 'btn-success');
                    $('#choose-record-{{$record->id}}').addClass( 'btn-danger');
                    $('#choose-record-{{$record->id}}').html( 'batal');
